# ChatGPT Model+Harness Cog

A **Model+Harness Cog** using Codex CLI with your ChatGPT login. It does not expose subscription tokens or pretend to provide bare model inference.
...
Run `codex login` and choose ChatGPT. `codex login status` confirms the login.
...
pixi run serve -- --binding ../cog-workbench/var/suite/<hash>-<revision>.json --port 8122
```
...
on every completion. The default port is 8122; the sibling providers default
to 8121 (cog-claude) and 8123 (cog-turn-harness).
...
The one model id served is `chatgpt/<binding_id>@<revision>`; a request naming
...
- **Loopback, and a token on every route.** The gateway binds `127.0.0.1` and
  ...
  `Origin`, and requires the bearer token even on `/health`. It performs the
  turn with **the owner's own vendor login**. It is **never a route for anyone
  else's subscription**.
...
The binding draft's `composition` is `model+harness`. Both provider and harness
identities are retained. The configured model is a requested selector; aliases may change and vendor internals are not attested.
